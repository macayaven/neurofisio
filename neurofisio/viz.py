...
_REGION_COLORS = {
    ...
    'FP1-F7': '#4dd0e1', 'F7-T3': '#4dd0e1', 'T3-T5': '#4dd0e1', 'T5-O1': '#4dd0e1',
    # Right temporal (channels 4-7): warm coral
    'FP2-F8': '#ff8a65', 'F8-T4': '#ff8a65', 'T4-T6': '#ff8a65', 'T6-O2': '#ff8a65',
    # Central (channels 8-13): amber
    'A1-T3': '#ffd54f', 'T3-C3': '#ffd54f', 'C3-CZ': '#ffd54f',
    'CZ-C4': '#ffd54f', 'C4-T4': '#ffd54f', 'T4-A2': '#ffd54f',
    # Left parasagittal (channels 14-17): soft green
    'FP1-F3': '#81c784', 'F3-C3': '#81c784', 'C3-P3': '#81c784', 'P3-O1': '#81c784',
    # Right parasagittal (channels 18-21): soft violet
    'FP2-F4': '#ce93d8', 'F4-C4': '#ce93d8', 'C4-P4': '#ce93d8', 'P4-O2': '#ce93d8',
}
...
def plot_eeg_segment(raw, start_sec=0, duration_sec=10, title='EEG Signal',
                     *, dark=False, colorize=False, annotations=None):
    ...
    # ── Figure setup ─────────────────────────────────────────────────────
    if dark:
        fig, ax = plt.subplots(figsize=(20, max(8, n_ch * 0.45)),
                               facecolor=_DARK_BG)
    ...
    _ANN_COLORS = {
        'seiz': '#ef5350', 'seizure': '#ef5350',
        'slow': '#ffa726', 'slowing': '#ffa726',
        'bckg': None,  # don't draw background
        'eyem': '#42a5f5', 'chew': '#ab47bc', 'musc': '#ab47bc',
        'shiv': '#78909c', 'elec': '#78909c',
        'spsw': '#ef5350', 'gped': '#ff7043', 'pled': '#ffa726',
        'artf': '#78909c',
    }
    if annotations:
        ymin = -scale * 0.5
        ...
        for ann in annotations:
            ...
            if a_stop <= start_sec or a_start >= start_sec + duration_sec:
                continue
            ax.axvspan(max(a_start, start_sec), min(a_stop, start_sec + duration_sec),
                       alpha=0.12 if dark else 0.08, color=color, zorder=0)
            # Label at top
            mid = (max(a_start, start_sec) + min(a_stop, start_sec + duration_sec)) / 2
            ax.text(mid, ymax, label, fontsize=7, ha='center', va='bottom',
    ...
    for i in range(n_ch):
        ...
        ax.text(times[0] - 0.2, offset, ch_name, fontsize=7.5,
                ha='right', va='center', color=color if colorize else text_color,
    ...
    if dark or colorize:
        ...
        ax.plot([bar_x, bar_x], [bar_y, bar_y + bar_uv * 1e-6],
                color=text_color, linewidth=1.5, zorder=5)
        ax.text(bar_x + duration_sec * 0.01, bar_y + bar_uv * 0.5e-6,
                f'{bar_uv} uV', fontsize=7, color=text_color,
